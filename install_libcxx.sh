...
echo "Compiler version: $COMPILER_VERSION_OUTPUT"
...
if [ ${VERSION} == $TRUNK_VERSION ]; then
    ...
    curl -O $LLVM_URL
    echo "downloading $LIBCXX_URL"
    curl -O $LIBCXX_URL
    echo "downloading $LIBCXXABI_URL"
    ...
    ls -lrt
    tar -xJf llvm-${VERSION}.src.tar.xz -C llvm-source --strip-components=1
    tar -xJf libcxx-${VERSION}.src.tar.xz -C llvm-source/projects/libcxx --strip-components=1
    tar -xJf libcxxabi-${VERSION}.src.tar.xz -C llvm-source/projects/libcxxabi --strip-components=1
fi
